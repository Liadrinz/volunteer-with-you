<view>
    ...
    <view class="cu-list menu" style="padding-bottom: 4rem;">
        <navigator class="cu-item" wx:for="{{activityList}}" wx:for-index="idx" wx:for-item="act"
            url="/pages/activityDetail/activityDetail?id={{act.id}}">
            <view class="content padding-tb-sm">
                ...
                <view wx:elif="{{actType === 'work'}}">
                    <view class="text-sm">
                        <text class="text-grey padding">发布时间: {{act.publishTime}}</text>
                    </view>
                    <view class="text-sm">
                        <text class="text-grey padding">报名时间: {{act.beginRegTime}}</text>
                    </view>
